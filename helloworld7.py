...
parzmid =   'u181624542f97966010e76c5a521cc979'
lumid =     'u71a8179068233d3c9bf05c927a3699ff'
killermid = 'u4f1a8e739a316e5f80317ad5a4a30dc4'
janmid =    'u82ddf006be2c51b44bb37302e2dfa0cf'
    ...
    try:
        ...
        if op.type == 17:
            ...
            if op.param1 == "c431a52f1131ba1d01ce91ada4bede4ba":
                dzx = line.getContact(op.param2)
                line.sendMessage(op.param1, "Hello " + str(dzx.displayName) + "!\n\nWelcome to " + str(ginfo.name) + ".")
                line.sendImage(op.param1,"/home/pi/Downloads/dl_bird.jpg")
        if op.type == 25:
            msg      = op.message
            text     = str(msg.text)
            msg_id   = msg.id
                ...
                if 'tagall' in text.lower():
                    ...
                    for g in gs.members:
                        if _nametarget == g.displayName:
                            ...
                            line.sendMessage(msg.to,msg.text,msg.contentMetadata)
                            line.sendMessage(msg.to,msg.text,msg.contentMetadata)
                            line.sendMessage(msg.to,msg.text,msg.contentMetadata)
                            line.sendMessage(msg.to,msg.text,msg.contentMetadata)
                            line.sendMessage(msg.to,msg.text,msg.contentMetadata)
                    ...
                    if members:
                        mids = []
                        if lumid in members: mids.append(lumid)
                        if killermid in members: mids.append(killermid)
                        if janmid in members: mids.append(janmid)
                        if not mids:
                            line.sendMessage(to, 'No bank managers in this chat')
                        else:
                            parsed_len = len(mids)//20+1
                            result = '╭─「 Bank Managers 」\n'
                            mention = '@zeroxyuuki\n'
                            no = 0
                            for point in range(parsed_len):
                                mentionees = []
                    ...
                    line.sendMessage(msg.to,"https://darklegendsgaming.com")
                elif '/youtube' in text.lower():
                    line.sendMessage(msg.to,"https://www.youtube.com/channel/UCPW1tTAU9hLqKIl-yzhnqYw")
                elif '/facebook' in text.lower():
                    line.sendMessage(msg.to,"https://www.facebook.com/Dark-Legends-Gaming-347046466196913")
                elif '/teamspeak' in text.lower():
                    line.sendMessage(msg.to,"fekhole.teamspeak3.com:3171")
                elif '/discord' in text.lower():
                    line.sendMessage(msg.to,"https://discord.gg/3pe7qFx")
                elif '/family' in text.lower():
                    line.sendMessage(msg.to,family)
                    ...
                    textt = text[11:]
                    transtext = translator.translate(textt, dest='en')
                    line.sendMessage(msg.to,transtext.text)
                    ...
                    if 'MENTION' in msg.contentMetadata.keys() != None:
                        ...
                        for mention in mentionees:
                            try:
                                line.sendMessage(msg.to,str(mention['M']))
                            except Exception as e:
                                pass
                elif '/instagram' in text.lower():
                    line.sendMessage(msg.to,"https://www.instagram.com/dark_legends_gaming")
                elif '/twitter' in text.lower():
                    line.sendMessage(msg.to,"https://www.twitter.com/DLG_Online")
                elif '/donate' in text.lower():
                    line.sendMessage(msg.to,donate)
                elif '/merch' in text.lower():
                    line.sendMessage(msg.to,"https://shop.spreadshirt.com/dark-legends-gaming")
                elif '/invitationlist' in text.lower():
                    textt = text[16:]
                    texttl = textt.lower()
                    texts = textt[7:]
                    ...
                    res += '\n╰───「 Hello World 」'
                    ress.append(res)
                    if '/invitationlist' == text.lower():
                        for res in ress:
                            line.sendMessage(to, parsingRes(res).format_map(SafeDict(key=setKey.title())))
                            line.sendMessage(to, textt)
                            line.sendMessage(to, texttl)
                            line.sendMessage(to, texts)
                        ...
                        for res in ress:
                            line.sendMessage(to, parsingRes(res).format_map(SafeDict(key=setKey.title())))
#                            line.sendMessage(to, texxt)
#                            line.sendMessage(to, texxtl)
#                            line.sendMessage(to, texts)
                elif '/grouplist' in text.lower():
                    textt = text[11:]
                    texttl = textt.lower()
                    gids = line.getGroupIdsJoined()
                    gnames = []
